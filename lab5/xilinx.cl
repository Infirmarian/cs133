...
void CnnKernel(__constant float* input, __constant float* weight,
 ...
 float C[kImSize][kImSize] // buffer of output
 ;
    ...
    for (int j = 0; j < kNum; ++j) {
      ...
      for (int h = 0; h < kImSize; ++h) {
        ...
        for (int w = 0; w < kImSize; ++w) {
          ...
          for (int p = 0; p < kKernel; ++p) {
            for (int q = 0; q < kKernel; ++q)
              tmp += weight[i*kNum*kKernel*kKernel + j*kKernel*kKernel + p*kKernel+q] * input(j,h + p,w + q);
          }
          C[h][w] += tmp;
        }
      }
...
//   for (int h = 0; h < kOutImSize; ++h) {
//       for (int w = 0; w < kOutImSize; ++w) {
//         output[i*kOutImSize*kOutImSize + h*kOutImSize + w] = max( max(0.f, 
